...
def run_model(model, tspan, n_sim=1000, simulator='gpu_ssa', precision=np.float32):
    ...
    if simulator == 'gpu_ssa':
        ...
        traj = sim.run_one_step(tspan=tspan, number_sim=n_sim, threads=16)
...
if __name__ == "__main__":
    ...
    # traj = run_model(earm_model, tspan, 1000, simulator='cutauleaping')
    # quit()
    # traj = run_model(ras_model, tspan, 100, simulator='gpu_ssa')

    if opt == 'earm':
        sol = ScipyOdeSimulator(earm_model)
        tspan = np.linspace(0, 10000, 101)
        name = 'cSmac_total'
        traj = sol.run(tspan=tspan)
        plt.figure(0)
        plt.plot(tspan, traj.observables[name], '--o', color='black')

        traj = run_model(earm_model, tspan, 5000, simulator='gpu_ssa')
        traj1 = run_model(earm_model, tspan, 5000, simulator='gpu_ssa', precision=np.float64)
        traj2 = run_model(earm_model, tspan, 5000, simulator='cutauleaping')
    ...
    def plot(traj, num, title, color='blue'):
        ...
        plt.plot(t, x.mean(axis=1), 'b', color=color, lw=3, label=title)
